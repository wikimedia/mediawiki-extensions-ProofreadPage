...
class ProofreadPagePageTest extends ProofreadPageTestCase {
	...
	public function testGetCustomCss() {
		$index = ProofreadIndexPageTest::newIndexPage( 'Test', "{{\n|CSS= width:300px; \n}}" );
		$this->assertEquals(
			'width:300px;', self::newPagePage( 'Test.jpg', $index )->getCustomCss()
		);
		...
		$this->assertEquals(
			'/* insecure input */', self::newPagePage( 'Test.jpg', $index )->getCustomCss()
		);
		...
		$this->assertEquals(
			'width:300px;&lt;style&gt;', self::newPagePage( 'Test.jpg', $index )->getCustomCss()
		);
	}
}
