...
trait LegacyParserTrait {
	...
	public function getPageNumberExpression( $expression ): string {
		return $this->parser->recursiveTagParse( $expression );
	}
	...
	public function addTemplate( $title, $articleId, $revId ): void {
		$this->parser->getOutput()->addTemplate( $title, $articleId, $revId );
	}
	...
	public function makeLink( $title, $text, $options = [] ): string {
		return $this->parser->getLinkRenderer()->makeLink( $title, $text, $options );
	}
	...
	public function addImage( $title, $timestamp, $sha1 ): void {
		$this->parser->getOutput()->addImage( $title, $timestamp, $sha1 );
	}

	/**
	 * Set extension data in the parser output.
	 *
	 * @param string $key The data key
	 * @param mixed $value The data value
	 */
	public function setExtensionData( $key, $value ): void {
		$this->parser->getOutput()->setExtensionData( $key, $value );
	}
}
