...
function proofreadPageSetup() {
	if(self.proofreadPageViewURL) {
	   ...
	   cell1.setAttribute("style","width:1em");
	   cell2 = document.createElement("td");   
	   cell2.appendChild(container);
	   cell2.setAttribute("valign","top");
	   t_row.appendChild(cell0);
	   t_row.appendChild(cell1);
	   t_row.appendChild(cell2);
	   t_body.appendChild(t_row);
	   table.appendChild(t_body);

	   isEditing = document.URL.indexOf("action=edit") > 0 || document.URL.indexOf("action=submit") > 0;
	   if(isEditing) { 
		   text = document.getElementById("wpTextbox1"); 
		   text.setAttribute("style", "width:100%");      //this seems to be set by monobook already...
		   text.setAttribute("style", "height:100%");
	   }
	   else { text = document.getElementById("bodyContent"); }
	   if(!text) return;
	   ...
	   if(copywarn){ f.insertBefore(table,copywarn);} else {f.appendChild(table);}
	  }
}
